...
describe('buildUrl', () => {
  ...
  it('replaces single parameter in template', () => {
    ...
    expect(buildUrl(template, request, params)).toBe('https://example.com/api/12345')
  })
  ...
  it('replaces multiple parameters in template', () => {
    ...
    expect(buildUrl(template, request, params)).toBe('https://example.com/api/users/12345')
  })
  ...
  it('encodes parameter values with special characters', () => {
    ...
    expect(buildUrl(template, request, params)).toBe('https://example.com/api/hello%20world')
  })
  ...
  it('handles numeric parameter values', () => {
    ...
    expect(buildUrl(template, request, params)).toBe('https://example.com/api/6359437/6020942')
  })

  it('defaults to "intern" subdomain when request is not for intern or ansatt', () => {
    const request = new Request('https://example.com')

    const template = 'https://{subdomain}.example.com/api'
    const params = {}
    expect(buildUrl(template, request, params)).toBe('https://intern.example.com/api')
  })
  ...
  it('combines subdomain and parameters in same template', () => {
    ...
    expect(buildUrl(template, request, params)).toBe('https://ansatt.nav.no/sak/789')
  })

  it('handles templates without parameters', () => {
    const template = 'https://example.com/api/endpoint'
    const params = {}
    expect(buildUrl(template, request, params)).toBe('https://example.com/api/endpoint')
  })
  ...
  it('encodes special URL characters in parameters', () => {
    ...
    expect(buildUrl(template, request, params)).toBe('https://example.com/search/key%3Dvalue%26other%3Ddata')
  })

  it('handles empty string parameter values', () => {
    const template = 'https://example.com/api/{id}'
    const params = { id: '' }
    expect(buildUrl(template, request, params)).toBe('https://example.com/api/')
  })

  it('handles zero as numeric parameter', () => {
    const template = 'https://example.com/page/{page}'
    const params = { page: 0 }
    expect(buildUrl(template, request, params)).toBe('https://example.com/page/0')
  })

  it('replaces same parameter multiple times if it appears in template', () => {
    const template = 'https://example.com/{id}/details/{id}'
    const params = { id: '123' }
    expect(buildUrl(template, request, params)).toBe('https://example.com/123/details/123')
  })
})
