...
describe('handler-discovery', () => {
  ...
  describe('getAvailableHandlers', () => {
    it('should return available handler mappings based on folder structure', () => {
      ...
      const hasVurderSamboer = handlers.some(
        h => h.behandlingHandler === 'alderspensjon-soknad' && h.aktivitetHandler === 'vurder-samboer',
      )
  ...
  describe('findRouteForHandlers', () => {
    it('should find route for valid handler combination', () => {
      ...
      const route = findRouteForHandlers('alderspensjon-soknad', 'vurder-samboer')
      expect(route).toBe('alderspensjon-soknad/vurder-samboer')
    })

    it('should return null for non-existent handler combination', () => {
      const route = findRouteForHandlers('unknown-behandling', 'unknown-aktivitet')
  ...
  describe('buildAktivitetRedirectUrl', () => {
    ...
    it('should build correct redirect URL for valid handlers', () => {
      const url = buildAktivitetRedirectUrl('6359437', '6020942', mockBehandling, mockAktivitet)

      expect(url).toBe('/behandling/6359437/aktivitet/6020942/alderspensjon-soknad/vurder-samboer')
    })
    ...
    it('should return null when behandling has no handlerName', () => {
      ...
      const url = buildAktivitetRedirectUrl('6359437', '6020942', behandlingWithoutHandler, mockAktivitet)
    ...
    it('should return null when no route mapping exists', () => {
      ...
      const url = buildAktivitetRedirectUrl('6359437', '6020942', mockBehandling, unknownAktivitet)
  ...
  describe('getHandlersForBehandling', () => {
    it('should return handlers for existing behandling', () => {
      ...
      // Should include at least the vurder-samboer handler we know exists
      const hasVurderSamboer = handlers.some(
        h => h.aktivitetHandler === 'vurder-samboer' && h.routePath === 'alderspensjon-soknad/vurder-samboer',
      )
      expect(hasVurderSamboer).toBe(true)
    })
  ...
  describe('getHandlerNamesFromPath', () => {
    it('should extract handler names from valid URL path', () => {
      const names = getHandlerNamesFromPath('/behandling/6359437/aktivitet/6020942/alderspensjon-soknad/vurder-samboer')
    ...
    it('should return null for URL with unknown handler combination', () => {
      const names = getHandlerNamesFromPath('/behandling/123/aktivitet/456/unknown-behandling/unknown-aktivitet')
      expect(names).toBeNull()
    })
