...
export function createAktivitetApi({ request, behandlingId, aktivitetId }: AktivitetApiParams) {
  ...
  return {
    hentAttestering: <T>() => serverFetch<T>(request, url('attestering')),
    ...
    hentGrunnlagsdata: <T>() => serverFetch<T>(request, url('grunnlagsdata')),

    hentInput: <T>() => serverFetch<T>(request, url('input')),
